...
req.onreadystatechange = function() {
	if (req.readyState == 4 && req.status == 200) {
		...
		for(grid in obj){
			utterances[obj[grid][0]]=obj[grid][1]
			links[obj[grid][0]]=obj[grid][2]
			slide_number[obj[grid][0]]=obj[grid][3]
		}
...
function add(i,j) {
	...
	if(links[key][i][j]!="blank")
	{
		switch(links[key][i][j])
		{
			...
			case "1": key="top_page"; break;
			default: key=(links[key][i][j]);document.main.src="images/CK15+."+pad(slide_number[key])+".png";
		}
	}
}
