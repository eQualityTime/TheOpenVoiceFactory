...
def get_row(in_num):
    ...
    return row[in_num] if in_num in row else row[min(row.keys(), key=lambda k: abs(k-in_num))] 


def get_column(in_num):
            #from http://stackoverflow.com/a/7934624/170243 
    return column[in_num] if in_num in column else column[min(column.keys(), key=lambda k: abs(k-in_num))] 
...
for slide in prs.slides:
    ...
    print "xxxxxx"
    #print dir(slide.shapes.title)
    #title_placeholder = slide.shapes.title
    #print title_placeholder.text
    print "New Slide!"
